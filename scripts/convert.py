    ...
    if checkSum(raw_data[0:10], raw_data[10]):
        ...
        if raw_data[1] == 0x50:
            ...
            msg += '20{:0>2d}-{:0>2d}-{:0>2d} {:0>2d}:{:0>2d}:{:0>2d}.{:0>3d}\t'.format(
                raw_data[2], raw_data[3], raw_data[4], raw_data[5], raw_data[6], raw_data[7], ms)
        elif raw_data[1] == 0x51:
            if headflag == 1:
                headmsg += 'ax(g)\tay(g)\taz(g)\t'
            msg += "{:.3f}\t{:.3f}\t{:.3f}\t".format(
                val[0] / 2048.0, val[1] / 2048.0, val[2] / 2048.0)
        elif raw_data[1] == 0x52:
            if headflag == 1:
                headmsg += 'wx(deg/s)\twy(deg/s)\twz(deg/s)\t'
            msg += "{:.3f}\t{:.3f}\t{:.3f}\t".format(
                val[0] / 32768.0 * 2000.0,
                val[1] / 32768.0 * 2000.0,
                val[2] / 32768.0 * 2000.0)
        elif raw_data[1] == 0x53:
            if headflag == 1:
                headmsg += 'AngleX(deg)\tAngleY(deg)\tAngleZ(deg)\t'
            msg += "{:.3f}\t{:.3f}\t{:.3f}\t".format(
                val[0] / 32768.0 * 180.0,
                val[1] / 32768.0 * 180.0,
                val[2] / 32768.0 * 180.0)
